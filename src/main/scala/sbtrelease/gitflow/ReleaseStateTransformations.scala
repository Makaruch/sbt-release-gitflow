...
object ReleaseStateTransformations {
  ...
  class Helper(
    ...
    def findReleaseBranch(searchRemote: Boolean, artifactName: String): Option[String] = {
      info(s"trying to find release branch with name ${artifactName}")
      //непонятно почему тут был поиск s"release/${artifactName}"
      val result =
        JGit.WorkingDir.findBranch("release").map(_.getName)
      result match {
    ...
    def checkoutBranch(branchName: String): Unit = {
      ...
      JGit.WorkingDir.checkoutExisting(branchName)
    ...
    def addAndCommitVersionFile(commitMessage: String): Unit = {
      ...
      JGit.WorkingDir.addToIndex(versionFile)
    ...
    def runPublish() = Step.unit { st =>
      info("runPublish not working, now skipping")
      if (!skipPublish || true) {
        ...
        //todo вот тут все не работает блеа
